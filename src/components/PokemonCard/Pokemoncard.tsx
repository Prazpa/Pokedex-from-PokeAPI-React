...
const Pokemoncard = ({image,name,id,types} : PokemonCardProps ) => {
    return (
        <div className="rounded-[20px] overflow-hidden shadow dark:bg-gray-800 dark:border-gray-700 p-[16px] bg-[#253641] max-w-[275px] ">
            ...
            <div className="pt-5">
                ...
                <div className='flex gap-2 justify-end mt-[16px]'>
                    {types.map((item) => {
                        return (
                            <span className={`badge-type-${item.type.name} px-[14px] capitalize py-1 rounded-[16px]`}>{item.type.name}</span>
                        )
                    })}                    
                </div>
            </div>
